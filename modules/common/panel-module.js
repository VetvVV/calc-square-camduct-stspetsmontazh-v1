(function(){
  ...
  const canSaveProject=["client","admin"].includes(role);
        ...
        <div class="layout">
          <div class="left-col">
            ...
            <section class="result-panel">
              ...
              ${canSaveProject?`<button class="add-btn" type="button" id="addBtn">${editIndex!==null?t.update:t.add}</button>`:""}${!canSaveProject&&role==="guest"&&editIndex===null?`<div class="guest-note">${t.guest}</div>`:""}
            </section>
          </div>
  ...
  function optionsHtml(){return cfg.type==="table"?`<p class="tab-empty">${t.notReady}</p>`:(canViewFormulas?`<p class="tab-empty">${t.formula}</p>`:"")}
